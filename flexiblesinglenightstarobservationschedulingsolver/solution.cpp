...
using namespace flexiblesinglenightstarobservationschedulingsolver;
        ...
        Time start_time)
{
    ...
    if (target.deadline < start_time + observation_time) {
        throw std::runtime_error(
                "target.deadline < start_time + target.observation_time");
    }
    if (start_time > target.meridian) {
        throw std::runtime_error(
                "start_time > target.meridian");
    }
    if (start_time + observation_time < target.meridian) {
        throw std::runtime_error(
    ...
    targets_[target_id] = 1;
    current_time_ = start_time + observation_time;
    profit_ += target.profits[observation_time_pos];
}
    ...
    if (verbose >= 2) {
        ...
        for (TargetId observation_pos = 0;
            ...
            os
                ...
                << std::setw(12) << observation.start_time + target.observation_times[observation.observation_time_pos]
                << std::endl;
        }
    }
...
void Solution::write(std::string certificate_path) const
{
    ...
    if (!file.good()) {
        throw std::runtime_error(
                "flexiblesinglenightstarobservationschedulingsolver::Solution::write\n"
